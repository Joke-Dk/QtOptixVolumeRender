...
Material DefineGlassMaterial( Context& m_context)
{
	...
	glassMaterial["index_of_refraction"  ]->setFloat( 1.4f );
...
optix::Material DefineFogMaterial( optix::Context& m_context, int kindBound, float indexRefraction)
{
	// Set up glass material
	...
	fogMaterial["index_of_refraction"]->setFloat(indexRefraction);
